...
$CSVdo=Import-Csv .\Documents\FORVAssoc.csv -Delimiter "♥" -Encoding utf8NoBOM
$CSVnot=Import-CSV .\Documents\FORVAnteckningar.csv -Delimiter "♥" -Encoding utf8NoBOM
$CSVsam=Import-CSV .\Documents\FORVSamband.csv -Delimiter "♥" -Encoding utf8NoBOM
...
"@
    ...
    #Restriktion 
    if(($arende.andra_beh -ne "W")-or($arende.arende_mening -match "Sekretess")-or($arende.arende_mening -match "(Sekr)")-or($arende.prim_nr -match "046 Donationer, fonder")){
        $RestriktionAr = @"
...
foreach($anteckning in $CSVnot){
    ...
    if($diarienummera -eq $anteckningdnr){
        ...
        if(($anteckningtext -match "Diarienummer : \d\d/SN\d\d\d\d")-or($anteckningtext -match "Diarienummer : \d\d/SO\d\d\d\d")){
            $ExtraIDAr += @"
...
"@
            ...
            if(($handling.andra_beh -ne "W")-or($handling.atgard_text -match "Sekretess")-or($handling.atgard_typ -match "ANSÖKAN")){
                $RestriktionHa = @"
...
Read-Host -Prompt "Byt namn på FÖRV-fil till FORV FILTYPER Validera XML-dokumentet. Replace & med &amp; i notepad++. Sök först efter &amp; för att inte lägga dubbla. Lägg också till filtyper"
############################################################################
...
<mets:mets xmlns:mets="http://www.loc.gov/METS/"
           ...
           LABEL="Diarieförda ärenden Förvaltarstyrelsen, ur Ciceron Diabas."
           TYPE="ERMS"
           ...
           ext:APPRAISAL="No"
           ext:ARCHIVALNAME="FÖRVALTARSTYRELSEN"
           ext:PACKAGENUMBER="1"
  ...
  <mets:metsHdr CREATEDATE="$DateTimeExtended"
    ...
    <mets:agent ROLE="ARCHIVIST"
                TYPE="ORGANIZATION">
      <mets:name>FÖRVALTARSTYRELSEN</mets:name>
      <mets:note>Local:10701</mets:note>
    </mets:agent>
    ...
    <mets:agent ROLE="ARCHIVIST"
                TYPE="OTHER"
                OTHERTYPE="SOFTWARE">
      <mets:name>Ciceron Diabas</mets:name>
...
Compress-Archive -Path .\Documents\Arkivexport\Paket\Leveranspaket\* -DestinationPath ".\Documents\Script\FORVDiabas$DateYYYYMMDD" -CompressionLevel Optimal -Force
