...
$CSVdo=Import-Csv .\Documents\ONSDiariumAssocdata.csv -Delimiter "¤" -Encoding utf8NoBOM
$CSVnot=Import-CSV .\Documents\ONSDiariumAnteckningar.csv -Delimiter "¤" -Encoding utf8NoBOM
$CSVsam=Import-CSV .\Documents\ONSDiariumSamband.csv -Delimiter "¤" -Encoding utf8NoBOM
...
"@
    ...
    $RestriktionAr = @"
<Restriktion Typ="Sekretess"><ForklarandeText>Ärendet ligger i sekretessdiarium, pröva sekretess</ForklarandeText></Restriktion>
"@
...
foreach($anteckning in $CSVnot){
    ...
    if($diarienummera -eq $anteckningdnr){
        ...
        #Förekommer
        if($anteckningtext -match "Diarienummer : \d\d/ON\d\d\d\d"){
            $ExtraIDAr += @"
...
"@
            ...
            #ONS är sekretessdiarium, sätter allt till sekretess
            $RestriktionHa = @"
<Restriktion Typ="Sekretess"><ForklarandeText>Handlingen ligger i sekretessdiarium, pröva sekretess</ForklarandeText></Restriktion>
"@
...
<mets:mets xmlns:mets="http://www.loc.gov/METS/"
           ...
           LABEL="Diarieförda ärenden Omsorgsnämnden, sekretessdiarium, ur Ciceron Diabas."
           TYPE="ERMS"
           ...
           ext:APPRAISAL="No"
           ext:ARCHIVALNAME="OMSORGSNÄMNDEN"
           ext:PACKAGENUMBER="1"
  ...
  <mets:metsHdr CREATEDATE="$DateTimeExtended"
    ...
    <mets:agent ROLE="ARCHIVIST"
                TYPE="ORGANIZATION">
      <mets:name>OMSORGSNÄMNDEN</mets:name>
      <mets:note>Local:10010</mets:note>
    </mets:agent>
...
Compress-Archive -Path .\Documents\Arkivexport\Paket\Leveranspaket\* -DestinationPath ".\Documents\Script\ONSDiabas$DateYYYYMMDD" -CompressionLevel Optimal -Force
