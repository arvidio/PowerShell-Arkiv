...
$CSVdo=Import-Csv .\Documents\DKDiariumAssocdata.csv -Delimiter "¤" -Encoding utf8NoBOM
$CSVnot=Import-CSV .\Documents\DKDiariumAnteckningar.csv -Delimiter "¤" -Encoding utf8NoBOM
$CSVsam=Import-CSV .\Documents\DKDiariumSamband.csv -Delimiter "¤" -Encoding utf8NoBOM
...
foreach($anteckning in $CSVnot){
    ...
    if($diarienummera -eq $anteckningdnr){
        ...
        if($anteckningtext -match "Diarienummer : \d\d/DK\d\d\d\d"){
            $ExtraIDAr += @"
...
<mets:mets xmlns:mets="http://www.loc.gov/METS/"
           ...
           LABEL="Diarieförda ärenden Drätselkontoret, ur Ciceron Diabas"
           TYPE="ERMS"
           ...
           ext:APPRAISAL="No"
           ext:ARCHIVALNAME="KOMMUNSTYRELSEN: DRÄTSELKONTORET"
           ext:PACKAGENUMBER="1"
  ...
  <mets:metsHdr CREATEDATE="$DateTimeExtended"
    ...
    <mets:agent ROLE="ARCHIVIST"
                TYPE="ORGANIZATION">
      <mets:name>KOMMUNSTYRELSEN: DRÄTSELKONTORET</mets:name>
      <mets:note>Local:10040</mets:note>
    </mets:agent>
...
Compress-Archive -Path .\Documents\Arkivexport\Paket\Leveranspaket\* -DestinationPath ".\Documents\Script\DKDiabas$DateYYYYMMDD" -CompressionLevel Optimal -Force
