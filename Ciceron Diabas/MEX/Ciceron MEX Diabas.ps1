...
$CSVdo=Import-Csv .\Documents\MEXDiariumAssocdata.csv -Delimiter "¤" -Encoding utf8NoBOM
$CSVnot=Import-CSV .\Documents\MEXDiariumAnteckningar.csv -Delimiter "¤" -Encoding utf8NoBOM
$CSVsam=Import-CSV .\Documents\MEXDiariumSamband.csv -Delimiter "¤" -Encoding utf8NoBOM
...
foreach($anteckning in $CSVnot){
    ...
    if($diarienummera -eq $anteckningdnr){
        ...
        #Förekommer ej i MEX
        if($anteckningtext -match "Diarienummer : \d\d/MEX\d\d\d"){
            $ExtraIDAr += @"
...
<mets:mets xmlns:mets="http://www.loc.gov/METS/"
           ...
           LABEL="Diarieförda ärenden Mark- och exploatering, ur Ciceron Diabas, arkivbildarskap oklart."
           TYPE="ERMS"
           ...
           ext:APPRAISAL="No"
           ext:ARCHIVALNAME="MARK- OCH EXPLOATERING"
           ext:PACKAGENUMBER="1"
...
Compress-Archive -Path .\Documents\Arkivexport\Paket\Leveranspaket\* -DestinationPath ".\Documents\Script\MEXDiabas$DateYYYYMMDD" -CompressionLevel Optimal -Force
