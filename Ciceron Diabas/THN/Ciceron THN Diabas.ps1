...
$CSVdo=Import-Csv .\Documents\THNDiariumAssocdata.csv -Delimiter "¤" -Encoding utf8NoBOM
$CSVnot=Import-CSV .\Documents\THNDiariumAnteckningar.csv -Delimiter "¤" -Encoding utf8NoBOM
$CSVsam=Import-CSV .\Documents\THNDiariumSamband.csv -Delimiter "¤" -Encoding utf8NoBOM
...
"@
    ...
    #Restriktion
    if(($arende.andra_beh -ne "W")-or($arende.arende_mening -match "Sekretess")-or($arende.arende_mening -match "Hemlig")-or($arende.arende_mening -match "OSL kap")){
        $RestriktionAr = @"
...
foreach($anteckning in $CSVnot){
    ...
    if($diarienummera -eq $anteckningdnr){
        ...
        #Förekommer inte i THN
        if($anteckningtext -match "Diarienummer : \d\d/THN\d\d\d"){
            $ExtraIDAr += @"
...
"@
            ...
            if(($handling.andra_beh -ne "W")-or($handling.atgard_text -match "Sekretess")-or($handling.atgard_text -match "Hemlig")-or($handling.atgard_text -match "OSL kap")){
                $RestriktionHa = @"
...
<mets:mets xmlns:mets="http://www.loc.gov/METS/"
           ...
           LABEL="Diarieförda ärenden Tiohundranämnden, ur Ciceron Diabas."
           TYPE="ERMS"
           ...
           ext:APPRAISAL="No"
           ext:ARCHIVALNAME="TIOHUNDRANÄMNDEN"
           ext:PACKAGENUMBER="1"
  ...
  <mets:metsHdr CREATEDATE="$DateTimeExtended"
    ...
    <mets:agent ROLE="ARCHIVIST"
                TYPE="ORGANIZATION">
      <mets:name>TIOHUNDRANÄMNDEN</mets:name>
      <mets:note>Local:10047</mets:note>
    </mets:agent>
...
Compress-Archive -Path .\Documents\Arkivexport\Paket\Leveranspaket\* -DestinationPath ".\Documents\Script\THNDiabas$DateYYYYMMDD" -CompressionLevel Optimal -Force
