...
$CSVdo=Import-Csv .\Documents\SNDiariumAssocdata.csv -Delimiter "♥" -Encoding utf8NoBOM
$CSVnot=Import-CSV .\Documents\SNDiariumAnteckningar.csv -Delimiter "♥" -Encoding utf8NoBOM
$CSVsam=Import-CSV .\Documents\SNDiariumSamband.csv -Delimiter "♥" -Encoding utf8NoBOM
...
"@
    ...
    #Restriktion 
    if(($arende.andra_beh -ne "W")-or($arende.arende_mening -match "Sekretess")-or($arende.arende_mening -match "(Sekr)")){
        $RestriktionAr = @"
...
Read-Host -Prompt "FILTYPER Validera XML-dokumentet. Replace & med &amp; i notepad++. Sök först efter &amp; för att inte lägga dubbla. Lägg också till filtyper"
############################################################################
...
<mets:mets xmlns:mets="http://www.loc.gov/METS/"
           ...
           LABEL="Diarieförda ärenden Socialnämnden, ur Ciceron Diabas."
           TYPE="ERMS"
           ...
           ext:APPRAISAL="No"
           ext:ARCHIVALNAME="SOCIALNÄMNDEN"
           ext:PACKAGENUMBER="1"
  ...
  <mets:metsHdr CREATEDATE="$DateTimeExtended"
    ...
    <mets:agent ROLE="ARCHIVIST"
                TYPE="ORGANIZATION">
      <mets:name>SOCIALNÄMNDEN</mets:name>
      <mets:note>Local:10022</mets:note>
    </mets:agent>
...
Compress-Archive -Path .\Documents\Arkivexport\Paket\Leveranspaket\* -DestinationPath ".\Documents\Script\SNDiabas$DateYYYYMMDD" -CompressionLevel Optimal -Force
