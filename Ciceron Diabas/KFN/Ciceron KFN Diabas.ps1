...
$CSVdo=Import-Csv .\Documents\KFNDiariumAssoc.csv -Delimiter "¤" -Encoding utf8NoBOM
$CSVnot=Import-CSV .\Documents\KFNDiariumAnteckningar.csv -Delimiter "¤" -Encoding utf8NoBOM
$CSVsam=Import-CSV .\Documents\KFNDiariumSamband.csv -Delimiter "¤" -Encoding utf8NoBOM
...
foreach($anteckning in $CSVnot){
    ...
    if($diarienummera -eq $anteckningdnr){
        ...
        if($anteckningtext -match "Diarienummer : \d\d/KFN\d\d\d"){
            $ExtraIDAr += @"
...
<mets:mets xmlns:mets="http://www.loc.gov/METS/"
           ...
           LABEL="Diarieförda ärenden Kultur- och fritidsnämnden, ur Ciceron Diabas"
           TYPE="ERMS"
  ...
  <mets:metsHdr CREATEDATE="$DateTimeExtended"
    ...
    <mets:agent ROLE="ARCHIVIST"
                TYPE="ORGANIZATION">
      <mets:name>KULTUR- OCH FRITIDSNÄMNDEN</mets:name>
      <mets:note>Local:10018</mets:note>
    </mets:agent>
