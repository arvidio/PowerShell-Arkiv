...
$CSVdo=Import-Csv .\Documents\RTJKSDiariumAssocdata.csv -Delimiter "¤" -Encoding utf8NoBOM
$CSVnot=Import-CSV .\Documents\RTJKSDiariumAnteckningar.csv -Delimiter "¤" -Encoding utf8NoBOM
$CSVsam=Import-CSV .\Documents\RTJKSDiariumSamband.csv -Delimiter "¤" -Encoding utf8NoBOM
...
foreach($anteckning in $CSVnot){
    ...
    if($diarienummera -eq $anteckningdnr){
        ...
        #Förekommer
        if($anteckningtext -match "Diarienummer : \d\d/RTJ\d\d\d"){
            $ExtraIDAr += @"
...
<mets:mets xmlns:mets="http://www.loc.gov/METS/"
           ...
           LABEL="Diarieförda ärenden Räddningstjänsten, ur Ciceron Diabas. Under Tekniska nämnden mellan 1995-01-01 och 2002-12-31, efter det kommunstyrelsen."
           TYPE="ERMS"
...
Compress-Archive -Path .\Documents\Arkivexport\Paket\Leveranspaket\* -DestinationPath ".\Documents\Script\RTJKSDiabas$DateYYYYMMDD" -CompressionLevel Optimal -Force
