...
$CSVdo=Import-Csv .\Documents\BUNDiariumAssocdata.csv -Delimiter "¤" -Encoding utf8NoBOM
$CSVnot=Import-CSV .\Documents\BUNDiariumAnteckningar.csv -Delimiter "¤" -Encoding utf8NoBOM
$CSVsam=Import-CSV .\Documents\BUNDiariumSamband.csv -Delimiter "¤" -Encoding utf8NoBOM
...
foreach($anteckning in $CSVnot){
    ...
    if($diarienummera -eq $anteckningdnr){
        ...
        if(($anteckningtext -match "Diarienummer : \d\d/BUN\d\d\d")-or($anteckningtext -match "Diarienummer : \d\d/BU\d\d\d\d")){
            $ExtraIDAr += @"
                    ...
                    if(('NULL' -eq $HandlingsLagrdata)-or($null -eq $HandlingsLagrdata)){
                        ...
                        foreach($fil in $filer){
                            if($fil.Extension -eq $fil.Name){
                                ...
                                if($testpath -eq $true){
                                    "$nypath finns redan, lägg på lite"
...
<mets:mets xmlns:mets="http://www.loc.gov/METS/"
           ...
           LABEL="Diarieförda ärenden Barn- och utbildningsnämnden, ur Ciceron Diabas"
           TYPE="ERMS"
           ...
           ext:APPRAISAL="No"
           ext:ARCHIVALNAME="BARN- OCH UTBILDNINGSNÄMNDEN"
           ext:PACKAGENUMBER="1"
  ...
  <mets:metsHdr CREATEDATE="$DateTimeExtended"
    ...
    <mets:agent ROLE="ARCHIVIST"
                TYPE="ORGANIZATION">
      <mets:name>BARN- OCH UTBILDNINGSNÄMNDEN</mets:name>
      <mets:note>Local:10027</mets:note>
    </mets:agent>
...
Compress-Archive -Path .\Documents\Arkivexport\Paket\Leveranspaket\* -DestinationPath ".\Documents\Script\BUNDiabas$DateYYYYMMDD" -CompressionLevel Optimal -Force
