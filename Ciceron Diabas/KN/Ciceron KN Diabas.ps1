...
$CSVdo=Import-Csv .\Documents\KNDiariumAssocdata.csv -Delimiter "¤" -Encoding utf8NoBOM
$CSVnot=Import-CSV .\Documents\KNDiariumAnteckningar.csv -Delimiter "¤" -Encoding utf8NoBOM
$CSVsam=Import-CSV .\Documents\KNDiariumSamband.csv -Delimiter "¤" -Encoding utf8NoBOM
...
foreach($anteckning in $CSVnot){
    ...
    if($diarienummera -eq $anteckningdnr){
        ...
        #Förekommer ej i KFÄ
        if($anteckningtext -match "Diarienummer : \d\d/KN\d\d\d\d"){
            $ExtraIDAr += @"
...
<mets:mets xmlns:mets="http://www.loc.gov/METS/"
           ...
           LABEL="Diarieförda ärenden Kulturnämnden, ur Ciceron Diabas"
           TYPE="ERMS"
           ...
           ext:APPRAISAL="No"
           ext:ARCHIVALNAME="KULTURNÄMNDEN"
           ext:PACKAGENUMBER="1"
  ...
  <mets:metsHdr CREATEDATE="$DateTimeExtended"
    ...
    <mets:agent ROLE="ARCHIVIST"
                TYPE="ORGANIZATION">
      <mets:name>KULTURNÄMNDEN</mets:name>
      <mets:note>Local:10035</mets:note>
    </mets:agent>
...
Compress-Archive -Path .\Documents\Arkivexport\Paket\Leveranspaket\* -DestinationPath ".\Documents\Script\KNDiabas$DateYYYYMMDD" -CompressionLevel Optimal -Force
