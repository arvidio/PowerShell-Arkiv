...
$CSVdo=Import-Csv .\Documents\KFÄDiariumAssocdata.csv -Delimiter "¤" -Encoding utf8NoBOM
$CSVnot=Import-CSV .\Documents\KFÄDiariumAnteckningar.csv -Delimiter "¤" -Encoding utf8NoBOM
$CSVsam=Import-CSV .\Documents\KFÄDiariumSamband.csv -Delimiter "¤" -Encoding utf8NoBOM
...
foreach($anteckning in $CSVnot){
    ...
    if($diarienummera -eq $anteckningdnr){
        ...
        #Förekommer ej i KFÄ
        if($anteckningtext -match "Diarienummer : \d\d/KFÄ\d\d\d"){
            $ExtraIDAr += @"
...
<mets:mets xmlns:mets="http://www.loc.gov/METS/"
           ...
           LABEL="Diarieförda ärenden Kommunalförbundet ägarsamverkan, ur Ciceron Diabas"
           TYPE="ERMS"
           ...
           ext:APPRAISAL="No"
           ext:ARCHIVALNAME="FRITIDSNÄMNDEN"
           ext:PACKAGENUMBER="1"
           ext:SYSTEMTYPE="KOMMUNALFÖRBUNDET ÄGARSAMVERKAN I NORRTÄLJES SJUKVÅRD OCH OMSORG">
  <mets:metsHdr CREATEDATE="$DateTimeExtended"
    ...
    <mets:agent ROLE="ARCHIVIST"
                TYPE="ORGANIZATION">
      <mets:name>KOMMUNALFÖRBUNDET ÄGARSAMVERKAN I NORRTÄLJES SJUKVÅRD OCH OMSORG</mets:name>
      <mets:note>Local:10702</mets:note>
    </mets:agent>
...
Compress-Archive -Path .\Documents\Arkivexport\Paket\Leveranspaket\* -DestinationPath ".\Documents\Script\KFADiabas$DateYYYYMMDD" -CompressionLevel Optimal -Force
