...
$CSVdo=Import-Csv .\Documents\BSNDiariumAssocdata.csv -Delimiter "¤" -Encoding utf8NoBOM
$CSVnot=Import-CSV .\Documents\BSNDiariumAnteckningar.csv -Delimiter "¤" -Encoding utf8NoBOM
$CSVsam=Import-CSV .\Documents\BSNDiariumSamband.csv -Delimiter "¤" -Encoding utf8NoBOM
...
foreach($anteckning in $CSVnot){
    ...
    if($diarienummera -eq $anteckningdnr){
        ...
        if($anteckningtext -match "Diarienummer : \d\d/BSN\d\d\d"){
            $ExtraIDAr += @"
...
<mets:mets xmlns:mets="http://www.loc.gov/METS/"
           ...
           LABEL="Diarieförda ärenden Barn- och skolnämnden, ur Ciceron Diabas"
           TYPE="ERMS"
           ...
           ext:APPRAISAL="No"
           ext:ARCHIVALNAME="BARN- OCH SKOLNÄMNDEN"
           ext:PACKAGENUMBER="1"
  ...
  <mets:metsHdr CREATEDATE="$DateTimeExtended"
    ...
    <mets:agent ROLE="ARCHIVIST"
                TYPE="ORGANIZATION">
      <mets:name>BARN- OCH SKOLNÄMNDEN</mets:name>
      <mets:note>Local:10003</mets:note>
    </mets:agent>
...
Compress-Archive -Path .\Documents\Arkivexport\Paket\Leveranspaket\* -DestinationPath ".\Documents\Script\BSNDiabas$DateYYYYMMDD" -CompressionLevel Optimal -Force
