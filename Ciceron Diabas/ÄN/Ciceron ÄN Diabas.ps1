...
$CSVdo=Import-Csv .\Documents\ÄNDiariumAssocdata.csv -Delimiter "¤" -Encoding utf8NoBOM
$CSVnot=Import-CSV .\Documents\ÄNDiariumAnteckningar.csv -Delimiter "¤" -Encoding utf8NoBOM
$CSVsam=Import-CSV .\Documents\ÄNDiariumSamband.csv -Delimiter "¤" -Encoding utf8NoBOM
...
foreach($anteckning in $CSVnot){
    ...
    if($diarienummera -eq $anteckningdnr){
        ...
        if($anteckningtext -match "Diarienummer : \d\d/ÄN\d\d\d\d"){
            $ExtraIDAr += @"
...
<mets:mets xmlns:mets="http://www.loc.gov/METS/"
           ...
           LABEL="Diarieförda ärenden Äldrenämnden, ur Ciceron Diabas"
           TYPE="ERMS"
  ...
  <mets:metsHdr CREATEDATE="$DateTimeExtended"
    ...
    <mets:agent ROLE="ARCHIVIST"
                TYPE="ORGANIZATION">
      <mets:name>ÄLDRENÄMNDEN</mets:name>
      <mets:note>Local:10067</mets:note>
    </mets:agent>
...
Compress-Archive -Path .\Documents\Arkivexport\Paket\Leveranspaket\* -DestinationPath ".\Documents\Script\ANDiabas$DateYYYYMMDD" -CompressionLevel Optimal -Force
